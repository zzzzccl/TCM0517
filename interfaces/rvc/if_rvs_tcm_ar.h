...
struct if_rvs_tcm_ar
{
    dbgtag_t debug_tag;

    uint8_t  burst : 2;  // Burst type. Fixed 1: INCR.
    uint32_t addr  : 32; // TCM byte address.
    uint8_t  size  : 3;  // Bytes per beat = 2^size. Fixed 5, 256 bits.
    ...
    void printInterface(std::ofstream& outfile, uint32_t portId) const
    {
        ...
        outfile << " (qos) = 0x"   << std::hex << qos << std::endl;
    }

    friend std::ostream& operator<<(std::ostream& os, const if_rvs_tcm_ar& req)
    {
